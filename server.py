...
def find_afterparties():
    ...
    payload = {'q': query, 'sort_by': sort, 
              'location.address': location, 'location.within': distance}

    response = requests.get(
    "https://www.eventbriteapi.com/v3/events/search", params=payload,
    headers = {
        "Authorization": "Bearer " + os.getenv('EVENTBRITE_TOKEN'),
    },
    verify = True,  # Verify SSL certificate
    )
    data = response.json()['events'][0]['name']['text']
    print data
